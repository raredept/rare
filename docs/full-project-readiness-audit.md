...
- A imagem principal de `Supreme Bag` é um PNG de aproximadamente 2,86 MB.
- `ProductMedia` usa `<img>` direto, sem `srcset` ou pipeline de otimização; o atributo `sizes` sozinho não reduz o download. Isso é risco de performance mobile, não bloqueio funcional.
...
- Otimizar imagens grandes para WEBP/AVIF e gerar variantes responsivas.
- Adotar `next/image` ou CDN de transformação para cards e produto.
...
- Otimização responsiva de mídia é insuficiente.
...
| P2 | Otimização de mídia | PNG de 2,86 MB prejudica mobile e Core Web Vitals. | R2 `Content-Length: 2863011`; `<img>` sem `srcset`. | Gerar WEBP/AVIF responsivo e pipeline de imagem. | Dev | Lighthouse e bytes transferidos menores. | Melhoria de código |
